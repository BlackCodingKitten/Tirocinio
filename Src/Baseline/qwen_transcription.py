...
def build_prompt(choice_0: str, choice_1: str, transcript: str) -> str:
    ...
    """
    return(
        "You are a binary selector assistant. No explanation, no punctuation, no preamble.\nTi viene fornita la trascrizione automatica dell'audio di un video.\n"
        "La trascrizione è in lingua italina, può contenere errori, omissioni o parole sbagliate.\n"
        "Usa solo ed esclusivamente le informazioni che riesci ad ottenere dalla trascrizione per scegliere la descrizione corretta.\n\n"
        f"Trascrizione:\n{transcript}\n\n"
        f"Scegli la descrizione corretta:\n"
        f"0. {choice_0}\n"
        f"1. {choice_1}.\n"
        f"Answer with exactly {1} or {0} , senza segni di punteggiatura o altre parole limitati alla selezione di uno dei due.\n"
    )
...
def main() -> None:
    ...
    # Output files
    predictions_output_path = Path("Data/ModelResponse/WithTranscription/qwen_mc_transcription.json")
    metrics_report_output_path = Path("Data/ModelResponse/WithTranscription/qwen_mc_transcription_metrics_report.txt")
    evaluation_output_dir = Path("Data/ModelResponse/WithTranscription/transcription_evaluation")
