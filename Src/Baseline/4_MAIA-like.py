...
PROMPT_TEMPLATE_VERSION = "maia_like_eval_video_plus_transcript_mc_01_v1"
...
def extract_transcription_text(payload: Any) -> str:
    ...
    if isinstance(payload, dict):
        for key in ("text", "transcription", "generated_transcription", "generated_text"):
...
def build_transcription_index(transcriptions_json: JsonDict) -> Dict[str, str]:
    index: Dict[str, str] = {}

    for raw_key, payload in transcriptions_json.items():
        transcription = extract_transcription_text(payload)
        ...
        for candidate in candidate_keys:
            if candidate:
                index[candidate] = transcription

    return index


def resolve_transcription(video_id: str, transcription_index: Dict[str, str]) -> str:
    ...
    for candidate in candidate_keys:
        transcription = transcription_index.get(candidate)
        if transcription:
...
def run_inference(
    ...
    for batch_idx, batch_examples in enumerate(chunks(list(examples), batch_size), start=1):
        ...
        for example in batch_examples:
            transcription = resolve_transcription(example.video_id, transcription_index)
            transcriptions.append(transcription)
            prompts.append(build_prompt(example.choice_0, example.choice_1, transcription))
        ...
        for example, transcription, prompt, raw_output in zip(
            batch_examples,
            transcriptions,
            prompts,
            raw_outputs,
...
def save_evaluation_artifacts(metrics_summary: JsonDict, output_dir: str | Path) -> None:
    ...
    workbook_path = output_dir / "4_MAIA-like_evaluation_summary.xlsx"
...
def main() -> None:
    ...
    predictions_output_path = out_dir / "qwen_mc_video_plus_transcript_predictions_by_video.json"
    flat_predictions_output_path = out_dir / "qwen_mc_video_plus_transcript_predictions_flat.json"
    metrics_output_path = out_dir / "qwen_mc_video_plus_transcript_metrics.json"
    metrics_report_output_path = out_dir / "qwen_mc_video_plus_transcript_metrics_report.txt"
    ...
    transcriptions_json = load_json_file(config.transcriptions_path)
    transcription_index = build_transcription_index(transcriptions_json)
    if not transcription_index:
        raise ValueError("No valid transcriptions were found in the transcription JSON.")

    print(f"[INFO] Loaded {len(transcription_index)} transcription entries.")
