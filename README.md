...
A video does not only contain visual information: it also carries relevant **spoken content** that can significantly improve understanding.

For this reason, the project explores whether integrating **automatically transcribed textual information** into a multimodal benchmark can provide a more complete representation of video content and help analyze the extent to which multimodal models rely on one modality more than others.

This is particularly important for studying **unimodal collapse**, namely the tendency of multimodal models to overdepend on a single modality even when multiple sources of information are available.
